...
<template>
  <div class="flex gap-3">
    ...
    <div class="max-w-[80%] min-w-0">
      <!-- Text content -->
      <div v-if="getTextBlocks()" class="bg-gray-50 rounded-2xl rounded-tl-sm px-4 py-2.5 text-sm leading-relaxed">
        <MarkdownRenderer :content="getTextBlocks()" />
        <span v-if="message.pending" class="inline-block w-1.5 h-4 bg-gray-400 animate-pulse ml-0.5 align-middle" />
      </div>

      <!-- Waiting animation when pending but no content yet -->
      <div v-else-if="message.pending" class="bg-gray-50 rounded-2xl rounded-tl-sm px-4 py-3 inline-flex items-center gap-1">
